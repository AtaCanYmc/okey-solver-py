...
def run_solver_demo():
    ...
    for i, meld in enumerate(arrangement.melds):
        ...
        print(
            f"  Meld #{i + 1} ({meld.type.value}): [{tiles_str}] - score: {meld.score}"
        )
...
def run_vision_solver_demo():
    ...
    def mock_detect(frame):
        print(
            f"[Vision Pipeline] Detecting tiles in frame metadata size: {frame.width}x{frame.height}"
        )
        # Simulated detections representing a Yellow 7, Blue 7, and Red 7 (Per candidate)
        return [
            Detection(
                id="d1",
                bounds=BoundingBox(x=5, y=5, width=15, height=20),
                confidence=0.99,
                label="YELLOW-7",
            ),
            Detection(
                id="d2",
                bounds=BoundingBox(x=25, y=5, width=15, height=20),
                confidence=0.97,
                label="BLUE-7",
            ),
            Detection(
                id="d3",
                bounds=BoundingBox(x=45, y=5, width=15, height=20),
                confidence=0.98,
                label="RED-7",
            ),
        ]
    ...
    pipeline = DefaultVisionPipeline(detect_fn=mock_detect, classify_fn=mock_classify)
